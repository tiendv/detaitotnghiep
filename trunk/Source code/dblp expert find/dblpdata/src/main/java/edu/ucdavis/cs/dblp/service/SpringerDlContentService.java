...
public class SpringerDlContentService implements ContentService {
	...
	private String retrieveAbstract(Publication pub, Parser parser) 
		...
		if (abstractText.indexOf("Keywords:") >= 0) {
			// chop off Keywords and leave it up to the retrieveKeywords method to do the work
...
//		testPub.setEe("http://192.168.1.22/acm/conf/isi/ZhaoBC03.html");
	...
	private void retrieveTerms(Publication pub, 
		...
		NodeFilter filter = 
				new HasParentFilter(
					new AndFilter (
				    		new TagNameFilter("div"),
				    		new HasAttributeFilter("class", "AbstractPara")
				    		)
				);
	...
	public boolean accepts(Publication pub) {
		...
		if (StringUtils.isNotEmpty(ee)) {
			...
			if (ee.contains("doi") && (ee.contains("10.1023")
					|| ee.contains("10.1007"))) accept = true;
		}
		return accept;
	}
