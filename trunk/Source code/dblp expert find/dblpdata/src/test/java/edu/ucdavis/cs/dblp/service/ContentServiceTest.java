...
public class ContentServiceTest {
	...
	public void searchAndVerify() {
		String[] testIds = new String[]{
			"conf/isi/AbbasiC07",
			"conf/3dpvt/HussainON04",
			"conf/isi/AbbasiC05",
			"conf/RelMiCS/SamingerBKM06",
			"journals/aamas/AkninePS04",
			"journals/tcad/AntreichS87",
			"conf/3dim/BoehnenF05",
			"conf/isi/AbbasiC07",
		};
		for (String testStr : testIds) {
			Publication testPub = pubDao.findById(testStr); 
			
			assertTrue("pub not accepted by any contentservice - "+testPub.getKey(), 
					contentService.accepts(testPub));
